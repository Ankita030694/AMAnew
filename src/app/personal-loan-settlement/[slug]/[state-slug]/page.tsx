...
import { personalLoanBanks, PersonalLoanBank } from "@/data/personalLoanBanks";
...
export async function generateStaticParams() {
  ...
  personalLoanBanks.forEach((bank) => {
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string; "state-slug": string }> }) {
  ...
  const { title, description } = getBankStateSettlementSEO(bank.name, state.name, slug, stateSlug, false);
  ...
  return {
    ...
    keywords: [
      `${bank.name} personal loan settlement in ${state.name}`,
      `stop ${bank.name} recovery agents ${state.name}`,
      `legal action against ${bank.name} in ${state.name}`,
      `${state.name} personal loan debt settlement`,
      `${state.policeAuthority} complaint against bank`,
      `${state.highCourt} guidelines for recovery agents`
    ],
    alternates: {
      canonical: `https://www.amalegalsolutions.com/personal-loan-settlement/${bank.slug}/${state.slug}`,
    },
  };
}
...
export default async function BankStateSettlementPage({ params }: { params: Promise<{ slug: string; "state-slug": string }> }) {
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "item": `https://www.amalegalsolutions.com/personal-loan-settlement/${bank.slug}`
      },
      {
        "@type": "ListItem",
        "position": 4,
        "name": `Settlement in ${state.name}`,
        "item": `https://www.amalegalsolutions.com/personal-loan-settlement/${bank.slug}/${state.slug}`
      }
    ]
  };
  ...
  const articleSchema = {
    "@context": "https://schema.org",
    "@type": "Article",
    "headline": `${bank.name} Personal Loan Settlement in ${state.name}`,
    "description": `Comprehensive legal guide for settling ${bank.name} personal loan debt within ${state.name}'s jurisdiction.`,
  ...
  const faqSchema = {
    ...
    "mainEntity": [
      ...
      },
      {
        ...
        "acceptedAnswer": {
          "@type": "Answer",
          "text": `No, a personal loan default is a civil breach of contract, not a criminal offense. While ${bank.name} can initiate civil arbitration, they cannot file a criminal FIR for non-payment. Proceedings must adhere to the civil legal framework overseen by the ${state.highCourt}.`
        }
      },
      {
        "@type": "Question",
        ...
        "acceptedAnswer": {
          "@type": "Answer",
          "text": `For high-value debt recovery cases in ${state.name}, matters are typically handled by Debt Recovery Tribunals (DRT) with jurisdiction encompassing ${state.drtLocations}. However, unsecured personal loan debts rarely meet the ₹20 Lakh threshold required for DRT action.`
        }
      },
      {
        "@type": "Question",
      ...
      },
      {
        ...
        "acceptedAnswer": {
          "@type": "Answer",
          "text": `An NOC is an official document from ${bank.name} stating that your personal loan account has been fully closed and there are no further dues pending. Securing a physical NOC on bank letterhead is the most critical step to ensure you are permanently protected from future legal claims.`
        }
      },
      {
        "@type": "Question",
      ...
      {
        "@type": "Question",
        "name": `Can ${bank.name} deduct money from my salary account for my personal loan dues?`,
        "acceptedAnswer": {
          "@type": "Answer",
          "text": `If your salary account is with the same institution (${bank.name}), they hold a 'Banker's Lien' (Right of Set-Off) and may automatically deduct funds to cover the personal loan default. It is highly advised to move your primary banking to a different institution before initiating a settlement.`
        }
      }
    ]
  };
  ...
  const tocSections = [
    { id: "introduction", title: `Understanding ${bank.name} Debt in ${state.name}` },
    { id: "local-jurisdiction", title: `Legal Jurisdiction in ${state.name}` },
    { id: "lok-adalats-state", title: `${state.name} Lok Adalats` },
    { id: "police-complaints", title: `Anti-Harassment in ${state.name}` },
    { id: "what-is-settlement", title: "The Settlement Process" },
  ...
  const breadcrumbItems = [
    ...
    { label: `${bank.name}`, href: `/personal-loan-settlement/${bank.slug}` },
    { label: `${state.name}`, href: `/personal-loan-settlement/${bank.slug}/${state.slug}` },
  ];
        ...
        <header className="relative bg-[#1a202c] text-white">
          ...
          <div className="relative z-20 container mx-auto px-4 py-12 md:py-32 text-center">
            <h1 className="text-2xl md:text-4xl font-bold mb-4 md:mb-6 leading-tight mt-10">
              {bank.name} Personal Loan Settlement in <span className="text-[#D2A02A]">{state.name}</span>
            </h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[220px_1fr_280px] gap-8 items-start">
            ...
            <article className="min-w-0 bg-white p-3 md:p-12 rounded-2xl shadow-sm space-y-6 md:space-y-12">
              ...
              <section id="local-landscape" className="scroll-mt-32">
                ...
                <p className="text-sm md:text-lg leading-relaxed mb-4 text-gray-700">
                  {state.economicContext} Over the past few years, the accessibility of personal loans provided by {bank.name} has surged across {state.name}. While this financial inclusion benefits many, the aggressive algorithmic penalties imposed by top-tier banking institutions quickly trap borrowers who encounter sudden financial hardships. When a borrower in {state.name} misses a payment, the compounding interest - often exceeding 40% annualized - along with exorbitant late payment fees, rapidly inflates the outstanding balance far beyond the original principal.
                </p>
                <p className="text-sm md:text-lg leading-relaxed mb-4 text-gray-700">
              ...
              <section id="harassment-tactics" className="scroll-mt-32 mt-10">
                ...
                <p className="text-sm md:text-lg leading-relaxed mb-4 text-gray-700">
                  To understand more about avoiding debt traps across multiple accounts, refer to our comprehensive guide on <Link href="/multiple-credit-card-loan-settlement" className="text-[#D2A02A] hover:underline font-semibold">multiple personal loan loan settlement</Link>.
                </p>
              </section>
              ...
              <section id="police-complaints" className="scroll-mt-32 mt-10">
                ...
                <p className="text-sm md:text-lg leading-relaxed mb-4 text-gray-700">
                  For deeper context on how various banks handle this escalation, see our ranking of the <Link href="/top-credit-card-banks-offering-settlement-india" className="text-[#D2A02A] hover:underline font-semibold">top personal loan banks offering settlement India</Link>.
                </p>
              </section>
            ...
            <aside className="hidden lg:block space-y-8 sticky top-24">
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                <h3 className="text-xl font-bold text-gray-900 mb-6">Client Success Stories in {state.name}</h3>
                <div className="space-y-6">
                  {reviewSchema.review.map((rev: any, index: number) => (
                    <div key={index} className="bg-gray-50 p-4 rounded-lg border border-gray-100 relative">
                      ...
                      <p className="font-bold text-gray-900 text-sm">- Verified Client, {state.name}</p>
                    </div>
                  ))}
                </div>
              </div>
